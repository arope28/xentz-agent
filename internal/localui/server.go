...
func init() {
	...
	statusBytes, err := dashboardHTML.ReadFile("templates/status.gohtml")
	if err != nil {
	...
	configBytes, err := dashboardHTML.ReadFile("templates/config.gohtml")
	if err != nil {
...
func (s *Server) handleStatus(cfgPath string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if bOK {
			lastBackupTime = lr.TimeUTC
			backupStatus = fmt.Sprintf("%s (%d files, %s)", lr.Status, lr.FilesTotal, func(b int64) string { unit := []string{"B","KB","MB","GB","TB"}; if b<=0{return "0 B"}; i:=0; f:=float64(b); for f>=1024&&i<len(unit)-1{f/=1024;i++}; return fmt.Sprintf("%.1f %s",f,unit[i]) }(lr.BytesSent))
		} else {
